...
export const getBriefDisplayTitle = (name: string): string => {
  ...
  if (cleaned.length <= 45) return cleaned.replace(/[-–—,.:״"׳']+$/, "").trim();

  // חיתוך על גבול מילה עד 45 תווים
  const cut = cleaned.substring(0, 45);
  const lastSpace = cut.lastIndexOf(" ");
  const result = lastSpace > 0 ? cut.substring(0, lastSpace) : cut;
  return result.replace(/[-–—,.:״"׳']+$/, "").trim() + "…";
};

export type MissingStep = "filmed" | "edited" | "cover" | "published";
export type PriorityLevel = "P0" | "P1" | "P2" | "P3" | "P4" | "PLANNING";
export type RiskType = "publishing" | "production-deadline" | "production-early-warning" | "planning";
export type RecommendedAction = "verify-upload" | "film" | "edit" | "cover" | "schedule" | "none";
...
export type ProductionTaskInput = { contentId: string; taskName: string; filmed: string; edited: string; coverReady: string; deadline?: string; };
...
const computeRecommendedAction = (published: boolean, ready: boolean, days: number | null, filmed: string, edited: string, hasGantt: boolean): RecommendedAction => {
  if (published) return "none";
  if (!hasGantt) return "schedule";
  if (days !== null && days <= 0 && ready) return "verify-upload";
  if (filmed !== "כן") return "film";
  if (edited !== "כן") return "edit";
  if (ready) return "verify-upload";
  return "none";
};

const buildReason = (level: PriorityLevel, days: number | null, ready: boolean, filmed: string, edited: string, hasGantt: boolean, title: string): string => {
  if (!hasGantt) return `"${title}" כבר בהפקה אבל עדיין אין לו תאריך עלייה.`;
  if (level === "P0") {
    if (ready) return `"${title}" מתוכנן לעלות היום וכבר מוכן.`;
    if (filmed !== "כן") return `"${title}" מתוכנן לעלות היום ועדיין לא צולם.`;
    return `"${title}" מתוכנן לעלות היום ועדיין חסרה עריכה.`;
  }
 if (level === "P1") {
    if (ready) return `"${title}" עולה מחר וכבר מוכן — כדאי לוודא שהכל סגור.`;
    if (filmed !== "כן") return `"${title}" עולה מחר ועדיין לא צולם.`;
    return `"${title}" עולה מחר וכבר צולם — כדאי לסגור עריכה היום.`;
  }
  if (level === "P2") return filmed !== "כן" ? `"${title}" עולה בעוד יומיים ועדיין לא צולם.` : `"${title}" עולה בעוד יומיים — כדאי לסגור עריכה.`;
  if (level === "P3") return filmed !== "כן" ? `"${title}" עולה בעוד 3 ימים ועדיין לא צולם.` : `"${title}" עולה בעוד 3 ימים וחסרה עריכה.`;
  return filmed !== "כן" ? `"${title}" עולה בעוד כמה ימים ועדיין לא צולם — שווה להתחיל להניע.` : `"${title}" עולה בעוד כמה ימים. הצילום מאחורייך, כדאי לסגור עריכה.`;
};
...
export const computePriorityItems = (ganttItems: GanttItemInput[], productionTasks: ProductionTaskInput[]): ContentPriorityItem[] => {
  ...
  const ganttContentIds = new Set(ganttItems.map((g) => g.contentId).filter(Boolean));
  const results: ContentPriorityItem[] = [];

  for (const g of ganttItems) {
    if (g.status === "פורסם") continue;
    ...
    results.push({ contentId: g.contentId, displayTitle, ganttDate: g.date, daysUntilUpload: days, filmed: ef, edited: ee, coverReady, ganttStatus, isReadyToUpload: ready, isPublished, missingSteps, priorityLevel: level, riskType, recommendedAction, reason: buildReason(level, days, ready, ef, ee, true, displayTitle), cta: buildCta(recommendedAction, displayTitle) });
  }
  ...
  for (const task of productionTasks) {
    ...
    results.push({ contentId: task.contentId, displayTitle, ganttDate: null, daysUntilUpload: null, filmed, edited, coverReady, ganttStatus: "", isReadyToUpload: ready, isPublished: false, missingSteps, priorityLevel: "PLANNING", riskType: "planning", recommendedAction: "schedule", reason: buildReason("PLANNING", null, ready, filmed, edited, false, displayTitle), cta: buildCta("schedule", displayTitle) });
  }

  const order: Record<PriorityLevel, number> = { P0: 0, P1: 1, P2: 2, P3: 3, P4: 4, PLANNING: 5 };
  results.sort((a, b) => {
    const d = order[a.priorityLevel] - order[b.priorityLevel];
    if (d !== 0) return d;
    if (a.daysUntilUpload !== null && b.daysUntilUpload !== null) return a.daysUntilUpload - b.daysUntilUpload;
    return 0;
  });
